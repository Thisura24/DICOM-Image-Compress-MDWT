...
% image and a target MSE value. Here Adaptive quantization with Linear
...
A1extracted = CombinedMatrixRead(1 , 1:(coloumnsA1*rowsA1));

VHDextracted = CombinedMatrixRead(1 , (coloumnsA1*rowsA1 + 1):size(CombinedMatrixRead,2));

A1reshaped = reshape(A1extracted, rowsA1 , coloumnsA1);

A1filtNew = A1reshaped * q;

decodedVHD = run_length_encode(VHDextracted,'decode');

VHDreshaped = reshape(decodedVHD, rowsA1 , coloumnsA1*3);

V1reshaped = VHDreshaped(1:rowsA1 , 1:coloumnsA1) ;

H1reshaped = VHDreshaped(1:rowsA1 , (coloumnsA1 + 1) : coloumnsA1*2) ;

D1reshaped = VHDreshaped(1:rowsA1 , (coloumnsA1*2 + 1) : coloumnsA1*3) ;

A1deQuantized = A1reshaped * q;
V1deQuantized = V1reshaped * q;
H1deQuantized = H1reshaped * q;
D1deQuantized = D1reshaped * q;
...
dicomwrite(reconstructedImage,fileName+"decom.dcm",metadata);
