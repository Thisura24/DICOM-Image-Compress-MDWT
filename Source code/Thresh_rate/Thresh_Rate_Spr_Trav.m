...
% Here Adaptive quantization with Spiral Traversal is used.
...
for i = 1:numel(fileNames)
    ...
    for n= 0.2: 0.03 : 0.99
        ...
        reshapedVHD = spiralTraversal(VHD);

        rleEncodedVHD = run_length_encode(reshapedVHD,'encode');
        ...
        VHDTraversed = inverseSpiralTraversal(decodedVHD, rowsA1 , coloumnsA1*3);
        ...
        reconstedImage = dicomClass(idwt2(A1filtNew,H1filtNew,V1filtNew,D1filtNew,w));

        MSE = immse(img,reconstedImage);

        diffImage = imabsdiff(img,reconstedImage);
        meanDiff = mean(diffImage(:));

        [peaksnr, snr] = psnr(reconstedImage, img);
...
function output = spiralTraversal(matrix)
...
while rowStart <= rowEnd && colStart <= colEnd
    for col = colStart:colEnd
        ...
        output = [output , matrix(rowStart, col)];
    ...
    for row = rowStart:rowEnd
        ...
        output = [output , matrix(row, colEnd)];

    end
    colEnd = colEnd - 1;

    if rowStart <= rowEnd
        for col = colEnd:-1:colStart

            output = [output , matrix(rowEnd, col)];
    ...
    if colStart <= colEnd
        for row = rowEnd:-1:rowStart

            output = [output , matrix(row, colStart)];
...
function output = inverseSpiralTraversal(matrix,rows, cols)
...
while rowStart <= rowEnd && colStart <= colEnd
    for col = colStart:colEnd
        ...
        output(rowStart, col) = matrix(index);
    ...
    for row = rowStart:rowEnd
        ...
        output(row, colEnd) = matrix(index);
    ...
    if rowStart <= rowEnd
        for col = colEnd:-1:colStart
            ...
            output(rowEnd, col) = matrix(index);
    ...
    if colStart <= colEnd
        for row = rowEnd:-1:rowStart
            ...
            output(row, colStart) = matrix(index);
